...
def t_to_seconds(t_ns: np.ndarray) -> np.ndarray:
    """Convert int64 nanosecond timestamps to float seconds since the
    first sample (suitable for plot x-axes)."""
    if len(t_ns) == 0:
        return t_ns.astype(float)
    return (t_ns.astype(np.float64) - float(t_ns[0])) / 1e9
