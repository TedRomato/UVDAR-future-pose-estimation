...
def _compute_metrics(artifacts: dict) -> dict:
    ...
    if uvdar_baseline is not None:
        ...
        finite = np.isfinite(uvdar_val).all(axis=1)
        uvdar_val = uvdar_val[finite]
        Y_val_u   = Y_val[finite]
        uvdar_err_3d = np.linalg.norm(uvdar_val - Y_val_u, axis=1)
        uvdar_rmse   = _rmse(uvdar_err_3d)

        uvdar_rmse_axes = {}
        for i, ax in enumerate(axis_names):
            uvdar_rmse_axes[ax] = _rmse(uvdar_val[:, i] - Y_val_u[:, i])
